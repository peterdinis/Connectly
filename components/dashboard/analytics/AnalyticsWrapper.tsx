    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground">Comprehensive insights into your page performance</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="destructive" size="sm" onClick={handleClearAnalytics} className="gap-2">
                    ...
                    return (
                      <div key={day.date} className="space-y-2">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-bold text-blue-600">{day.views} views</span>
                        </div>
                        <motion.div
                ...
                <div className="space-y-3">
                    ...
                    .map((hour, index) => {
                      ...
                      return (
                        <div key={hour.hour} className="space-y-2">
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-muted-foreground font-medium">{timeLabel}</span>
                            <span className="font-bold text-purple-600">{hour.views} views</span>
                          </div>
                          <motion.div
                  ...
                  {summary.viewsByHour.every((h) => h.views === 0) && (
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground">No hourly data yet</p>
                    </div>
                  )}
                </div>
              </CardContent>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Most clicked links with engagement metrics</CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <div key={link.linkId} className="space-y-2">
                        <div className="flex items-center justify-between text-sm">
                          <div className="flex items-center gap-2 flex-1">
                            ...
                            <span className="font-medium truncate">{link.linkTitle}</span>
                          </div>
                          <span className="font-bold text-green-600 ml-2">{link.clicks} clicks</span>
                        </div>
                        <motion.div
                          className="h-3 bg-muted rounded-full overflow-hidden"
                          initial={{ width: 0 }}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b">
                        <th className="text-left py-3 px-2 font-semibold text-sm">Link Title</th>
                        <th className="text-right py-3 px-2 font-semibold text-sm">Clicks</th>
                        <th className="text-right py-3 px-2 font-semibold text-sm">CTR</th>
                        <th className="text-right py-3 px-2 font-semibold text-sm">Performance</th>
                      </tr>
                    </thead>
                    <tbody>
                      {summary.linkPerformanceComparison
                        ...
                        .map((link, index) => (
                          <motion.tr
                            ...
                            transition={{ delay: 0.5 + index * 0.05 }}
                          >
                            <td className="py-3 px-2 font-medium">{link.linkTitle}</td>
                            <td className="py-3 px-2 text-right font-semibold text-green-600">{link.clicks}</td>
                      ...
                      >
                        <div
                          className={`p-2 rounded-full ${
                            event.type === "view" ? "bg-blue-100 text-blue-600" : "bg-green-100 text-green-600"
                          }`}
                        >
                          {event.type === "view" ? (
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium">
                            {event.type === "view" ? "Page viewed" : `Clicked: ${event.linkTitle}`}
                          </p>
